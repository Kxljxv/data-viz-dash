            ...
            <header class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex flex-col">
                        <Typography variant="h4" class="text-foreground">Kreisverbände wählen</Typography>
                        <Typography variant="body" class="text-xs text-muted-foreground/40">Erstelle eine Gruppe basierend auf regionaler Herkunft.</Typography>
                    </div>
                </div>
                <div class="flex flex-col items-end">
                    <Badge variant="brand" class="px-4 py-1.5 font-bold tracking-wider mb-1">{selectedKvs.length} KV's</Badge>
                    <Typography variant="body" class="text-[9px] text-muted-foreground/20 uppercase tracking-widest">Ausgewählt</Typography>
                </div>
            </header>
            ...
            <div class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <div class="relative flex-1 group">
                        <div class="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground/20 group-focus-within:text-[hsl(var(--accent-pro-100))] transition-colors">
                            <IconSearch size={18} />
                        </div>
                        <input 
                            type="text"
                            ...
                            class="w-full bg-muted/10 border border-border/50 rounded-2xl py-4 pl-12 pr-4 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[hsl(var(--accent-pro-100))]/50 transition-all placeholder:text-muted-foreground/20 shadow-inner"
                        />
                    </div>
                    
                    <Button variant="outline" size="sm" onclick={toggleAll} class="h-auto py-4 px-6 rounded-2xl border-border/20 bg-muted/5 text-[10px] font-black uppercase tracking-widest hover:bg-muted/10 hover:border-border/40 transition-all">
                            ...
                            <div class="flex items-center gap-4 relative z-10">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300
                                    {selectedKvs.includes(kv) 
                                        ...
                                        : 'bg-muted/10 text-muted-foreground/20 group-hover:bg-muted/20 group-hover:text-muted-foreground/40'}">
                                    <IconMapPin size={18} />
                                </div>
                                <div class="flex flex-col text-left">
                                    <span class="text-xs font-bold {selectedKvs.includes(kv) ? 'text-foreground' : 'text-muted-foreground/60'} transition-colors">{kv}</span>
                                    <span class="text-[9px] text-muted-foreground/20 uppercase tracking-widest">Regionalverband</span>
                                </div>
                            </div>
                            
                            <div class="w-6 h-6 rounded-full border-2 transition-all duration-300 flex items-center justify-center relative z-10
                                {selectedKvs.includes(kv) 
                                    ? 'bg-[hsl(var(--accent-pro-100))] border-[hsl(var(--accent-pro-100))] text-oncolor-100 shadow-[0_0_10px_hsla(var(--accent-pro-100),0.5)] scale-110' 
                                    : 'border-border/40 text-transparent group-hover:border-border/60'}">
                ...
                <Button 
                    ...
                    class="flex-1 sm:flex-none px-12 h-12 text-[10px] font-black uppercase tracking-widest transition-all
                        {selectedKvs.length > 0 
                            ? 'bg-[hsl(var(--accent-pro-100))] text-oncolor-100 shadow-[0_0_20px_hsla(var(--accent-pro-100),0.3)] hover:bg-[hsl(var(--accent-pro-100))]/80' 
