...
for url_list in url_lists:
    for url in url_list:
        ...
        heading_list = extract_heading_from_section(url)
        if not heading_list:
            print(f"Warning: No heading found for {url}")
            continue
            
        full_heading = heading_list[0]
        print(f"Heading: {full_heading}")
        
        try:
            # Try to extract the ID from the heading (e.g., "L-01: ..." or "L-01 zu ...")
            id_heading = re.split(r"[:\s]", full_heading)[0]
        except Exception:
            id_heading = "unknown"

        print(f"ID Heading: {id_heading}")
        ...
        if url not in label_mapping:
            label_mapping[url] = full_heading
            print(f"Added new label for {url}: {full_heading}")

        final_heading = "-".join([id_session, label_mapping[url]])
        

        print(f"Final Heading: {final_heading}")
        print(f"Applicant: {applicant}")
        print(f"Supporters count: {len(supporters)}")
        print(f"URL: {url}")
        

        if len(supporters) > 1:
            url_data["application_id"] = application_id
            url_data["heading"] = final_heading
            url_data["applicant"] = applicant
            url_data["supporters"] = supporters
            url_data["url"] = url
            database[application_id] = url_data
        else:
            print(f"Warning: Not enough supporters found for {url}")
