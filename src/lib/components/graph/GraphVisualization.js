...
class GraphVisualization {
    constructor(projectName = null, canvasId = 'graph-canvas') {
    ...
    init() {
        ...
        // Use container size if possible, else window
        ...
        this.zoom = d3.zoom()
            ...
            .on('zoom', (event) => {
                ...
                // Dispatch zoom event for sync
                // Only dispatch if user interaction (avoid infinite loop)
    ...
    async loadData() {
        try {
            // Get project from property or URL parameters
    ...
    setupEventListeners() {
        ...
        window.addEventListener('aea-filter-change', this.handlers.filterChange);
        window.addEventListener('aea-view-action', this.handlers.viewAction);
        window.addEventListener('aea-group-update', this.handlers.groupUpdate);
        window.addEventListener('aea-density-update', this.handlers.densityUpdate);
    }
    ...
    render() {
        ...
        this.nodes.forEach(n => {
            ...
            // Ursprüngliche Radius-Formel: (sqrt(linkCount) * 1.5 + 0.25) * nodeSize
    ...
    centerGraph() {
        ...
        // Calculate scale to fit with 10% margin (to match DensityMap's 5% on each side)
        ...
        // Immediately set transform to avoid sync delay on first render
        this.transform = transform;
        
        // Dispatch zoom event for sync immediately
        window.dispatchEvent(new CustomEvent('aea-graph-zoom', {
...
// Export for Svelte and make available globally
