...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <Typography tag="h1" variant="h1" class="text-foreground">Graph-Visualisierungen</Typography>
            ...
            <a href="/dashboard/graph/project/{project.id}" class="block group">
                <Card interactive class="h-full card-glass">
                    {#snippet header()}
                        ...
                        <Typography variant="h3" class="group-hover:text-brand transition-colors text-foreground">{project.name}</Typography>
                        <Typography variant="label" class="line-clamp-2 mt-1 opacity-50">{project.description}</Typography>
                    {/snippet}
                    
                    <div class="flex items-center gap-4 text-xs font-modern-dense uppercase tracking-wider text-muted-foreground/40">
