...
PERSONS_DB = os.path.join(SCRIPT_DIR, "persons.sqlite")
...
def ensure_persons_schema(conn):
    cur = conn.cursor()
    cur.execute(
...
def build_databases(data):
    ...
    persons_conn = sqlite3.connect(PERSONS_DB)
    ensure_amendments_schema(amend_conn)
    ensure_persons_schema(persons_conn)
